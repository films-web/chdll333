...
{
	return (qboolean)syscall(70, entityIndex, state);
}
...
{
	return (qboolean)syscall(CG_G2_GETANIMFILENAMEINDEX, ghoul2, modelIndex, filename);
}
...
{
	return (qboolean)syscall(CG_G2_SETSKIN, ghoul2, modelIndex, customSkin);
}
...
{
	return (qboolean)syscall(CG_G2_GETANIM, ghoul2, modelIndex, boneName, currentTime, frame);
}

qboolean trap_G2API_SetSurfaceOnOff(void* ghoul2, const int modelIndex, const char* surfaceName, const int flags)
{
	return (qboolean)syscall(CG_G2_SETSURFACEONOFF, ghoul2, modelIndex, surfaceName, flags);
}
...
{
	return (qboolean)syscall(CG_G2_SETNEWORIGIN, ghoul2, modelIndex, boltIndex);
}
...
{
	return (qboolean)(syscall(CG_G2_SETGHOUL2MODELFLAGS, ghlInfo, flags));
}
...
{
	return (qboolean)(syscall(CG_G2_SETGHOUL2MODELFLAGSBYINDEX, ghoul2, index, flags));
}
